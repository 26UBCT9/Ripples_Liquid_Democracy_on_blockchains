...
Liquid democracy on Ethereum: soulbound voting rights, voting papers with yes/no matters in eight fixed topics, per topic one hop delegation with own vote override, optional unlinkable anonymous ballots (Semaphore v4, real Groth16 proofs in the browser), live tallies that become final when the voting window closes, and gasless voting through a government relayer (ERC-2771). Frontend in React with web3.js.

Architecture and design rationale: `liquid-voting-architecture.md` (v0.9).

## Lifecycle

A paper is **live the moment it is published**, and so are its results:

1. **Voting** (creation to `votingEnd`): vote Yes/No per matter with one click, or switch a topic to anonymous mode first, any time. Every ballot is tallied immediately; the running result is publicly visible (a deliberate trade-off: usability over tally secrecy, see the architecture document).
2. **Ended**: the tally is final at `votingEnd`; anyone can finalize.
...
If you restart the chain (terminal 1), always rerun the deploy and restart the relayer; the relayer detects a redeployed or restarted chain and says so instead of failing cryptically.
...
1. Pick **Government**. In *Citizen administration*, the demo voters are listed with one-click *Issue voting right* buttons (any other address works via the form below). In *Paper administration*, publish a paper with one or two matters; the default (voting +10 min) is fine.
2. Switch to **Alice**: the paper is already open. Vote Yes on a matter and watch it appear in *Results* immediately, or set a standing delegation for a topic first (it counts from the *next* paper, the UI says so).
3. Switch to **Ben**: pick *Go anonymous* on a topic, then vote. One signature derives the anonymous identity, the browser generates a real zero knowledge proof (a few seconds), and the relayer submits it from its own account: nothing on chain links the ballot to Ben, yet the choice cannot be flipped by the relayer (it is baked into the proof).
...
5. When the voting countdown ends, the tally is final. Finalize in *Results*.
...
- The vote itself is the proof's message: the relayer physically cannot flip your choice, only submit or refuse it (and refusing is mitigated by the self-paid fallback).
...
- The relayer submits anonymous ballots from its own account. Locally the deploy script deploys the real Semaphore v4 stack (PoseidonT3, verifier, Semaphore), so local demos verify genuine proofs on chain.

## On-chain data view (for demos and presentations)

The **On-chain data** tab shows the raw chain truth without any external explorer: every paper and ballot as decoded contract events (block, transaction hash, timestamp), and per transaction the decoded calldata. Clicking a gasless vote reveals the ERC-2771 envelope (relayer paid, voter signed, inner call attributed); clicking an anonymous vote reveals the full Groth16 proof payload (root, nullifier, message, eight proof points) with no voter address anywhere. Made for screenshots.
...
`npm test` runs 24 tests: soulbound registry, checkpointed delegations with the creation snapshot boundary, override arithmetic in every voting order with live tallies asserted (incl. the dual role voter), phase gating (live voting, frozen tally after close), double ballot guards in both directions, forwarder attribution and replay rejection, and a real proof suite that deploys the genuine Semaphore contracts, verifies Groth16 on chain (incl. a single member set), grows the group mid vote, and rejects outsider roots, reused nullifiers and relayer choice flips.
