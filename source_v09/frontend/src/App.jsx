...
export default function App() {
  ...
  // Deployment guard: if the chain still runs contracts from an older version
  // (e.g. it was not redeployed after an upgrade), calls like voteAnonymous or
  // votePublic silently hit non-existent functions. publicVoted only exists
  // since v0.8, so probing it detects the mismatch.
  useEffect(() => {
    if (!isDeployed) return;
    (async () => {
      try {
        await contracts().controller.methods.publicVoted(0, deployment.forwarder).call();
        setStaleDeployment(false);
      } catch {
    ...
    >
      ...
      {staleDeployment && (
        <p className="notice notice-err">
          The contracts on this chain are older than this frontend (or the chain was not redeployed). Run{" "}
          <code>npm run deploy:local</code>, restart the relayer, then reload this page.
        </p>
      )}
      ...
      {!account ? (
        ...
        <main>
          ...
          {tab === "chain" && <ChainView notify={notify} />}
        </main>
      )}
