...
export async function voteAnonymousBallot(account, paperId, topicId, matterId, choice) {
  ...
  let group;
  try {
    ({ group } = await syncGroup(paperId, topicId));
  } catch (e) {
    throw new Error(`Anonymity set sync failed: ${e.message}`);
  }
  if (group.members.length === 0) {
    throw new Error(
      "Anonymity set sync found no members. If the chain was redeployed: run deploy, restart the relayer, reload this page, and register again."
    );
  }
  if (!group.members.some((m) => m === identity.commitment)) {
    throw new Error(
      "This account's identity is not in the anonymity set of this topic. Register with this account first (identity is derived per account)."
    );
  }
  const { controller } = contracts();
  ...
  let proof;
  try {
    proof = await generateProof(identity, group, message, scope, depth, artifactsFor(depth));
  } catch (bundled) {
    // Bundled artifacts failed to load or prove: retry letting the library
    // fetch its own artifacts from the public CDN.
    try {
      proof = await generateProof(identity, group, message, scope, depth);
    } catch (cdn) {
      throw new Error(`Proof generation failed: ${cdn?.message || bundled?.message || "unknown prover error"}`);
    }
  }
  ...
  if (!res.ok) throw new Error(`Relayer rejected the ballot: ${body.error || `HTTP ${res.status}`}`);
  return body;
}
