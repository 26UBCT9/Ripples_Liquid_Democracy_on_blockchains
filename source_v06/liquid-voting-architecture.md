...
Because opting into anonymity and voting publicly now share one window, ordering attacks must be excluded in both directions: `anonMode` blocks a later public commit on the topic (as before), and a new `hasPublicBallot[paper][topic][voter]` flag, set on the first public commit, blocks a later anonymous registration for the same topic. Without it a voter could commit publicly, then register and cast an unlinkable second ballot that nobody can attribute and subtract.

During Voting only commitments are accepted, public or anonymous. During Reveal (votingEnd to revealEnd, the evaluation period) only reveals are accepted and tallies are computed. After revealEnd the paper is Ended; finalize freezes the result and emits an event the results view listens to.
...
**VoteController.** The core contract. It inherits `ERC2771Context`, holds tallies and the commit, reveal and override logic, and talks to the canonical deployed Semaphore instance. Semaphore assigns sequential group ids at `createGroup()`; the controller stores the mapping and exposes it through `groupIdOf(paperId, topicId)`.

```solidity
function createPaper(string title, uint48 votingEnd, uint48 revealEnd, MatterInput[] matters) external;
...
// effects: also sets hasPublicBallot[paper][topic][voter]
...
Local, Hardhat with chai. Tests in place (23 passing): SBT non transferability; delegation checkpoints return correct values at the snapshot, including a delegation set before paper creation (counts) and one set right after (applies to the next paper only); override arithmetic in every reveal order (delegate first, delegator first, delegate never reveals, and the dual role voter); double commit rejection; nullifier reuse rejection; anonMode blocks public commits per topic; registration reverts when any delegation exists for the topic at the snapshot and when the voter already voted publicly on the topic (double ballot guard); phase gating on every function of the live lifecycle; forwarder replay rejection with a reused nonce and a foreign signature; a delegation set through the forwarder is attributed to the signing voter and never to the relayer or forwarder; and a real proof suite that deploys the genuine Semaphore stack, verifies Groth16 proofs on chain, exercises mid vote group growth, and rejects outsider roots and swapped ballot commitments. For Semaphore, run fast suites against a mock verifier and one slow honest suite that deploys the contracts from `@semaphore-protocol/contracts` and generates real proofs.
