  ...
  const create = async () => {
    try {
      ...
      const data = controller.methods
        .createPaper(title, toTs(deadlines.votingEnd), toTs(deadlines.revealEnd), matters.map((m) => [m.topicId, m.text]))
        .encodeABI();
  ...
  return (
    <section className="panel">
      ...
      <p className="hint">
        ...
        moment it is published; standing delegations are snapshotted at that same moment. After the voting window the
        evaluation (reveal) period runs until the second deadline.
      </p>
      <label>
        Title
