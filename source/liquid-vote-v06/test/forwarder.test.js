...
describe("Gasless path (ERC-2771)", () => {
  ...
  it("carries a full gasless vote: commit and reveal as meta transactions", async () => {
    const { relayer, alice, forwarder, controller } = await loadFixture(deployFixture);
    const now = await time.latest();
    await controller.createPaper("P", now + 300, now + 400, [{ topicId: EDUCATION, text: "m" }]);
    const matterId = await controller.matterCount(); // voting is live immediately

    const salt = ethers.encodeBytes32String("s");
    const commitment = ethers.keccak256(
      abi.encode(["uint256", "address", "bool", "bytes32"], [matterId, alice.address, true, salt])
    );
    let data = controller.interface.encodeFunctionData("commitPublic", [matterId, commitment]);
    await forwarder.connect(relayer).execute(await signMeta(forwarder, alice, await controller.getAddress(), data));
    expect(await controller.publicCommitOf(matterId, alice.address)).to.equal(commitment);

    await time.increaseTo(now + 301); // Reveal (votingEnd passed)
    data = controller.interface.encodeFunctionData("revealPublic", [matterId, true, salt]);
    await forwarder.connect(relayer).execute(await signMeta(forwarder, alice, await controller.getAddress(), data));
    const m = await controller.getMatter(matterId);
    expect(m.yes).to.equal(1n);
    expect(await controller.publicRevealed(matterId, alice.address)).to.equal(true);
  });
});
