...
describe("M2: real Semaphore v4 with genuine Groth16 proofs", function () {
  ...
  it("proves for a single-member set: depth clamps from 0 to 1 (the fresh-voter case)", async () => {
    const s = await loadFixture(deployRealFixture);
    // A second paper where exactly one voter goes anonymous.
    const now = await time.latest();
    await s.controller.createPaper("Solo", now + 500, now + 900, [{ topicId: EDUCATION, text: "Only one?" }]);
    ...
    expect(solo.depth).to.equal(0); // the buggy input the frontend used to pass

    const depth = Math.max(1, solo.depth); // what the frontend does now
    const cS = anonCommitment(soloMatter, true, salt("solo"));
    const proofS = await generateProof(maiaId, solo, BigInt(cS), await s.controller.scopeOf(soloMatter), depth, artifactsFor(depth));
    await expect(s.controller.connect(s.relayer).commitAnonymous(soloMatter, proofS, cS)).to.emit(
      s.controller,
      "AnonymousCommit"
    );

    await time.increaseTo(now + 501);
    await s.controller.connect(s.relayer).revealAnonymous(soloMatter, 0, true, salt("solo"));
