 ...
 *                      public votes). The voter's EIP-712
 ...
 *                      Semaphore proof is the authorization and binds the
 *                      choice as its message, so the relayer cannot flip it.
 *
...
const FRIENDLY = {
  ...
  FailedCall: "the relayed call reverted",
};
...
app.post("/relay/meta", async (req, res) => {
  try {
    ...
    // Simulate the inner call exactly as the target will see it (ERC-2771
    // appends the voter to the calldata), so reverts keep their reason
    // instead of surfacing as the forwarder's opaque FailedCall.
    await provider.call({
      to: forwardRequest.to,
      from: deployment.forwarder,
      data: forwardRequest.data + forwardRequest.from.slice(2),
    });
...
app.post("/relay/anonymous", async (req, res) => {
  try {
    await ensureDeployment();
    const { matterId, proof, choice } = req.body;
    if (req.body.kind && req.body.kind !== "vote") {
      return res.status(400).json({ error: "kind must be vote" });
    }
    const p = {
    ...
    await controller.voteAnonymous.staticCall(matterId, p, Boolean(choice)); // pre-validate
    const tx = await sendResilient(() => controller.voteAnonymous(matterId, p, Boolean(choice)));
    const receipt = await tx.wait();
    res.json({ txHash: receipt.hash, blockNumber: receipt.blockNumber });
  } catch (e) {
