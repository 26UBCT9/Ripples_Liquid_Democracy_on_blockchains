...
// ----------------------------------------------- local anonymous-vote marker
// On-chain nothing links an anonymous ballot to the voter (by design), so the
// UI remembers locally that this browser already cast one. Purely cosmetic:
// the nullifier enforces vote-once on-chain regardless.

const anonVoteKey = (account, matterId) => `anonvote:${deployment.chainId}:${account.toLowerCase()}:${matterId}`;

export function storeAnonVote(account, matterId, choice) {
  localStorage.setItem(anonVoteKey(account, matterId), JSON.stringify({ choice }));
}

export function loadAnonVote(account, matterId) {
  const raw = localStorage.getItem(anonVoteKey(account, matterId));
